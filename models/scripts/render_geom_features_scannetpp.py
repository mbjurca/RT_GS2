...
from models.gaussian_splatting.utils.general_utils import safe_state
...
if __name__ == '__main__':
    ...
    for stage in ['test']:
        scene_stage_dir = os.path.join(root_dir, stage)
        scene_list = [os.path.join(scene_stage_dir, scene) for scene in os.listdir(scene_stage_dir) if os.path.isdir(os.path.join(scene_stage_dir, scene))]
    ...
    for scene_dir in tqdm(scene_list, desc='Processing scenes'):
        ...
        for view in tqdm(views, desc='Processing views'):
            ...
            features = rendering["render_object"]
            features = transforms.functional.resize(features, (480, 640)).detach().cpu().numpy()

            np.save(os.path.join(scene_dir, "features", view.image_name + ".npy"), features)


            feature_rgb = feature_to_rgb(rendering["render_object"].detach().cpu())
            ...
            rendering_color_uint8 = cv2.resize(rendering_color_uint8[:, :, [2, 1, 0]], (640, 480))
            cv2.imwrite(os.path.join(scene_dir, "render", view.image_name + ".png"), rendering_color_uint8)