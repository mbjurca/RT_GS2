...
from models.AsymFormer.src.mix_transformer import OverlapPatchEmbed, mit_b0
from models.AsymFormer.src.convnext import convnext_tiny
from thop import profile
from models.AsymFormer.src.MLPDecoder import DecoderHead
import os
...
class down_sample_block(nn.Module):
    def __init__(self, inc_depth, inc_rgb, block_num):
        ...
        if block_num != 0:
            ...
            self.depth_stem = OverlapPatchEmbed(in_chans=32, embed_dim=inc_depth)
...
class B0_T(nn.Module):
    ...
    def forward(self, image, depth):
        ...
        rgb_out, x_lin = self.Decoder(
            [depth_out1,
             depth_out2,
             depth_out3,
             depth_out])
        rgb_out = F.interpolate(rgb_out, size=input_shape, mode='bilinear', align_corners=False)
        x_lin = F.interpolate(x_lin, size=input_shape, mode='bilinear', align_corners=False)
        return rgb_out, x_lin
