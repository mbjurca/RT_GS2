...
class ScanNet_Semantic(data.Dataset):
    def __init__(self, root_dir, scene_names, id2name, color_mapping, class_weights=None, transforms=None, stage='train', split='train', task='semantic', finetune=False, compute_semantic_weights=False):
        ...
        if finetune:
            scene_list = [os.path.join(scene_split_dir, scene_names[0])]
        else:
            scene_list = [os.path.join(scene_split_dir, scene) for scene in scene_names]
        ...
        mapping_file = '/home/mihnea/paper/Semantic-Ray/data/scannet/scannetv2-labels.combined.tsv'
    ...
    def map_id(self, id):
        return self.class_mapping.get(id, -1) # Map to other class (otherwise to unlabeled)
    
    def map_inv_id(self, id):
        return self.inv_class_mapping.get(id)
    
    def map_id2color(self, id):
        return self.color_mapping.get(id, [248, 166, 116])
    ...
    def load_instances(self, scene_list):
        ...
        for scene in scene_list:
            ...
            if self.stage == 'train':
                ...
                self.list_instances.extend([(scene, name) for idx, name in enumerate(names)])
    ...
    def __getitem__(self, index):
        ...
        image_path = os.path.join(name[0], 'images', name[1] + '.jpg')
        render_path = os.path.join(name[0], 'render', name[1] + '.png')
        feature_path = os.path.join(name[0], 'features', name[1] + '.npy')
        semantic_path = os.path.join(name[0], 'label_filtered', name[1] + '.png')
        ...
        semantic_mask = self.scan2nyu[semantic_mask]
        ...
        return sample['rendering'], sample['rendered_features'], sample['semantic_mask'], sample['depth'], image, name[0], name[1]
